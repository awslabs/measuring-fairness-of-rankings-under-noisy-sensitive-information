...
from inclusive_search_fairness_measurement.rnd.wrapper import compute_rND, compute_rND_udf
...
class PySparkTestCase(unittest.TestCase):
    ...
    def test_individual_value(self):
        """
        Test rND metric computation for one ranking
        """
        ...
        res = compute_rND(rows[1]["asin_data"])
        self.assertAlmostEqual(res, 0.347, places=3)

    def test_udf(self):
        """
        Test UDF function for rND metric computation
        """
        dataframe = self.spark.read.csv(sample_data_path, header=True)
        dataframe = dataframe.withColumn('is_sensitive', F.when(F.col("attribute_values") == "Women", 1).otherwise(0))
        dataframe = dataframe.withColumn(
        ...
        dataframe = dataframe.groupby("keywords").agg(F.collect_list(F.col("asin_data")).alias("asin_data"))
        dataframe = dataframe.withColumn("rND", compute_rND_udf("asin_data"))
        dataframe.show()
        rows = dataframe.collect()
        res = rows[1]["rND"]
        self.assertAlmostEqual(res, 0.347, places=3)
