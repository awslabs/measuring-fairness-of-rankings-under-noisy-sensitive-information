...
class Script:
    ...
    def execute(spark, input, execParams):
        ...
        # Join against browse node assignments on asin
        ...
        # Join against D_MP_ASIN_ATTRIBUTES to fetch the item names
        itemNameTable = spark.read.table("D_MP_ASIN_ATTRIBUTES")
        tommyAsinDepName = tommyAsinWithDepartments.join(itemNameTable, on=["asin", "marketplace_id"], how="left")

        # Aggregate back to the level of asins (there might be multiple browse nodes assigned to one ASIN)
        tommyAsinDepNamegrouped = tommyAsinDepName.groupby(
            [
                "marketplace_id", "query_group_id", "session", "search_date", "asin", "organic_position", "keywords",
                "item_name"
            ]
        ).agg(F.collect_set('attribute_value').alias('attribute_values'))

        # Add the struct containing all the information required for computation of metrics
        tommyAsinDepNamegrouped = tommyAsinDepNamegrouped.withColumn("metric_data",
                             F.struct(F.col("attribute_values"), F.col("organic_position"),
                                      F.col("item_name"), F.col("asin")))

        # Aggregate back to the level of searches
        impressedAsinsDataset = tommyAsinDepNamegrouped.groupby(
            [
                "marketplace_id", "query_group_id", "session", "search_date", "keywords"
            ]
        ).agg(F.collect_set('metric_data').alias('list_metric_data'))
        # agg(F.collect_set(F.col("attribute_value")).alias("attribute_values"))
        # datasetlog(impressedAsinsDataset, "impressedAsinsDataset")
        return impressedAsinsDataset.sample(False, 0.001).coalesce(1).select(
            [
